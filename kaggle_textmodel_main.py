#!/usr/bin/env python3
"""
FULL MULTIMODAL TRAINING SCRIPT — IMPROVED
Roberta + Swin on ITSACRISIS + CRISISIMAGES
- Multi-task (t1, t2, t3t, t3s, t4)
- Focal loss with class weights per task
- Mixed precision (GPU)
...
def LOSS(o, y, class_weights=None, device="cpu"):
    total = 0.0
    for k in y:
        m = y[k] >= 0
        if not m.any():
        ...
        total = total + focal_loss(logits, targets, alpha=alpha, gamma=2.0)
    return total
    ...
    for ep in range(epochs):
        ...
        for B, Y in tqdm(TL, desc=f"Epoch {ep+1}/{epochs}"):
            ...
            with torch.cuda.amp.autocast(enabled=(device == "cuda")):
                out = model(B)
                loss = LOSS(out, Y, class_weights=class_weights, device=device)
